...
export class DigitalOceanProvider extends BaseChatProvider {
  ...
  async chat(messages, options = {}, onUpdate = null) {
    ...
    // Kimi models on DO hard-require temperature === 1 AND top_p === 0.95,
    // rejecting anything else with a 400 ("temperature must be 1 for this
    // model" / "top_p must be 0.95 for this model"). GPT-OSS has no such
    // constraint, which is why only the secondary (Kimi) agent was 400ing.
    // Pin both for Kimi; leave every other model to its own defaults.
    const isKimi = /kimi/i.test(model);
    const temperature = isKimi ? 1 : (options.temperature || 0.7);
    const topP = isKimi ? 0.95 : (options.topP ?? undefined);
  ...
  async streamChat(messages, params, onUpdate) {
    ...
    try {
      ...
      const isReasoningModel = model.includes('kimi') || model.includes('deepseek-r1');
