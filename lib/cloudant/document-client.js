...
export class CloudantClient {
  ...
  async handleRateLimit(operation, retryCount = 0) {
    try {
      ...
      // nano attaches the whole request to its errors — for a failed save,
      // the document itself (a group's private signing key, an account's
      // pairwise keys). Callers log errors, so drop it here.
