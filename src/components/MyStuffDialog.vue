        ...
        <q-tab-panels v-model="currentTab" animated>
          ...
          <q-tab-panel name="summary">
            ...
            >
              ...
              <!-- Phase 4: the auto-generated privacy-filtered copy, right
            ...
            <!-- Phase 4: Privacy Filtered Patient Summary (read-only).
                 Regenerated automatically every time the summary is Verified
                 or Edited; used to respond to sharing requests by default. -->
            <template v-else-if="summarySubTab === 'filtered'">
              <div v-if="privacyFilteredSummary && privacyFilteredSummary.text">
                <div class="text-caption text-grey-7 q-mb-sm">
                  Created automatically when you verify or edit your Patient
                  Summary — this copy is what MAIA shares by default when you
                  accept a patient-summary sharing request.
                  <template v-if="privacyFilteredSummary.createdAt">
                    Updated {{ getTimeAgo(privacyFilteredSummary.createdAt) }} ago.
                  </template>
                </div>
                <div
                  v-if="!privacyFilteredSummary.mappingCount"
                  class="text-caption text-orange-9 q-mb-sm"
                >
                  No privacy-filter names are configured (Workbook → Privacy
                  Filter), so this copy is identical to your Patient Summary —
                  review it carefully before accepting sharing requests.
                </div>
                <div
              ...
              <div v-else class="text-body2 text-grey-7 q-pa-md bg-grey-1 rounded-borders">
                No privacy-filtered summary yet. It is created automatically the
                first time you Verify (or Edit and save) your Patient Summary.
              </div>
            </template>
...
const loadPatientSummary = async () => {
  ...
  try {
    ...
    // Phase 4: the auto-generated privacy-filtered copy for the tab.
    privacyFilteredSummary.value = result.privacyFiltered || null;
...
const finishVerifySummary = () => {
  ...
  // (best-effort; the local state is already updated). Phase 4: verification
  // also regenerates the privacy-filtered copy server-side — pull it so the
  // Privacy Filtered tab reflects it without a reload.
  if (props.userId) {
    ...
    void fetch('/api/patient-summary/verify', {
      ...
      try {
        ...
        if (r.ok) {
          ...
          privacyFilteredSummary.value = j.privacyFiltered || null;
          psVerifiedAt.value = j.verifiedAt || null;
        }
      } catch { /* tab refreshes on next load */ }
