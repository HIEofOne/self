...
| **P6 — Group and member routing** | `/gnap/group/:groupId` (+ continue, answer). Relay copies + group attestations. Sealed answers. Same-host fast path through the same handler. Group request page. Member-to-member through `maia_to`, with member claims moved onto RFC 9421. Tally + one payment per request. `legacy-requests` off in the edition. Security doc §15.4 + Appendix A (I-24…I-31) | The group and one-door parts of §10.11. Two-host suite: the same request direct, same-host and cross-host decides identically. The relay stores no readable answer |
