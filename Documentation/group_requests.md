...
- **Left for P5b:** `instance_id`, credits in the interaction, a QR code for the link, and X25519 (needed only for group answers, P6). | A clinician-style run in a clean browser: request → verify email → ask → patient accepts → email → return → collect. Key is non-extractable. No PHI in any email |
