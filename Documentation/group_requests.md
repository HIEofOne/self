...
- **Left for P6b:** member-to-member through `maia_to` (member claims on RFC 9421, answers under Sent), and the security doc. | The group and one-door parts of §10.11. Two-host suite: the same request direct, same-host and cross-host decides identically. The relay stores no readable answer |
